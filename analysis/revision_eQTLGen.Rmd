...
We have 989 supporting SNPs for all pairs with ECT FDR < 0.2, when filter by gene num >= 20, SNP num >= 5 . There are 159 unique SNPs. 

For each trait, we extracted the SNPs with p-value < 5E-8, then use the collected SNPs as the pool for random SNP, sampled 10*159 SNPs from it, use them as the random SNPs
...
We focus on 22 supporting SNPs that are also trans-eQTLs in eQTLGen. To get the expected values, we choose 10/100/1000 “random” genes per SNP as control, and see how many gene-snp pairs can be found in eQTLGen.
