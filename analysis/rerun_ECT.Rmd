...
# Some highlighted traits

```{r fig.height= 20, fig.width=10, warning=F, message=F}

load("/project/xinhe/xsun/pathway_factor/analysis/1.ECT/results/summary_mbe_steiger.rdata")


p <- list()
for (traits in c("lymph","baso","ret")) {
  
  #print(traits)  
  
sum_traits <- df_all[df_all$trait == traits,]

# pwy <- c("05220","04060","04640","04010","03013","05235","04664","05166","05169","00230")
# selected <- sum_traits[sum_traits$pwy_code %in% pwy,]
...
selected <- selected[order(selected$lgectpval,decreasing = T),]
...
p[[length(p)+1]] <- ggplot(selected) +
  ...
  geom_text(
    data = selected,
    aes(lgectpval, y = pwy_name, label = num_snp_pass_cutoff),
    hjust = 0,
    nudge_x = 0.01,
    colour = "black",
    size = 3,
