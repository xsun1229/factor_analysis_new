...
for (i in 1:nrow(df_all_fdr01)) {  
  ...
  power_all$celltype <- types[celltype]
...
## Weak -- Bottom 8 from  0.1 < ECT FDR < 0.2


```{r, results='asis', warning=FALSE, message=FALSE, fig.height=6, fig.width=12}
#for (i in 2:2) {

df_all_fdr02 <- df_all_fdr02[order(df_all_fdr02$ECT_FDR_5suppSNP,decreasing = T),]
...
## Weak -- Bottom 15 from  0.1 < ECT FDR < 0.2
...
## Weak -- TOP22 from  0.1 < ECT FDR < 0.2
...
## Weak -- all pairs with results (22 + 15 = 37) from  0.1 < ECT FDR < 0.2
...
df_all_fdr02 <- df_all_fdr02[order(df_all_fdr02$ECT_FDR_5suppSNP,decreasing = T),]
df_all_fdr02_bottom <- df_all_fdr02[1:15,]

df_all_fdr02 <- df_all_fdr02[order(df_all_fdr02$ECT_FDR_5suppSNP,decreasing = F),]
df_all_fdr02_top <- df_all_fdr02[1:22,]

df_all_fdr02_topbottom <- rbind(df_all_fdr02_top, df_all_fdr02_bottom)

power_all_pairs <- c()
for (i in 1:nrow(df_all_fdr02_topbottom)) {  
  trait=df_all_fdr02_topbottom$trait[i]
  factor=df_all_fdr02_topbottom$factor[i]
  celltype=df_all_fdr02_topbottom$celltype[i]
