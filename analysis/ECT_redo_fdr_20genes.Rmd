---
title: "Effect Size Consistency Tests (SNP FDR < 0.2, FDR corrected, pathways with >=20 genes included)"
author: "XSun"
date: "2022-12-01"
...
2. For the pairs have multiple snps (>=3) passing FDR < 0.2 && corrected ACAT FDR < 0.2 or FET FDR <0.2, do ECT

3. Pathways with >= 20 genes were included here.
...
for (i in 1:length(path)){
  folder_path <- paste0("/project2/xinhe/xsun/CEDAR/",path[i],"/10.20genes_rerun/2.ECT/")
...
for (i in 1:length(type)){ 
  ...
  file_ect_results <- paste0("/project2/xinhe/xsun/CEDAR/",path[i],"/10.20genes_rerun/2.ECT/results/ECT_results.rdata")
  load(file_ect_results)
  
  file_ect_summary <- paste0("/project2/xinhe/xsun/CEDAR/",path[i],"/10.20genes_rerun/2.ECT/results/ECT_summary_table.rdata")
  ...
  p <- as.data.frame(plot_sum$pval_ECT)
  colnames(p) <- "p"
  p3[[i]] <-  ggplot(p, aes(x=p)) + geom_histogram(breaks = seq(0, 1, by = 0.05),color="white",fill = "steelblue")+ 
    theme_bw(base_line_size =0.3) +
    ggtitle(type[i]) + theme(plot.title = element_text(hjust = 0.5)) +
    labs(x = "ECT p-values",
         y = "Count") +
...
all3 <- grid.arrange(p3[[1]],p3[[2]],p3[[3]],p3[[4]],p3[[5]],p3[[6]], nrow = 2)
```
